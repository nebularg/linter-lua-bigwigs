...
local function parseLua(file)
	...
	for n, line in ipairs(lines) do
		...
		if line == "function mod:GetOptions()" then
			...
			if not opts then
				-- rip keys
				error(string.format("    %s:%d: Error parsing GetOptions! %s", file:match(".*/(.*)$") or file, n, err))
			else
				option_keys = opts
			end
		end
		...
		if checkForAPI(line) then
			...
			for i, k in next, keys do
				...
				if not option_keys[key] then
					error(string.format("    %s:%d: Invalid key! func=%s, key=%s", file:match(".*/(.*)$") or file, n, f, key))
					errors = true
				end
				keys[i] = key
			end
			...
			for _, c in next, tablize(color) do
				...
				elseif c and c ~= "nil" then
					...
					error(string.format("    %s:%d: Invalid color! func=%s, key=%s, color=%s", file:match(".*/(.*)$") or file, n, f, table.concat(keys, " "), c))
				end
			end
			...
			for _, s in next, tablize(sound) do
				...
				elseif s and s ~= "nil" then
					...
					error(string.format("    %s:%d: Invalid sound! func=%s, key=%s, sound=%s", file:match(".*/(.*)$") or file, n, f, table.concat(keys, " "), s))
				end
			end
		end
	end
...
if arg and arg[1] then
	...
	if path:sub(-1) ~= "/" and path:sub(-3) ~= "lua" then
		path = path .. "/"
	end
	path = path:gsub("^./", "")
